...
class SemiMarkovCRF(StructuredModel):

	def __init__(self,
				 prior=None, transition_matrix=None, clf=None,
	 			 n_states=2, n_features=None, inference_method='qpbo',
                 class_weight=None):
		# super(SemiMarkovCRF, self).__init__(**kwargs)
		self.size_psi = 2

		self.clf = clf
		self.prior = prior
		self.transition_matrix = np.maximum(transition_matrix, .000001)
	...
	def psi(self, x, y):
		...
		unary = self.clf.predict_proba(semi_markov_data)

		# print ""
		# print 'y', y
		# print 'unary', unary
		# print 's_u', np.sum(np.log([unary[i,y_] for i,y_ in enumerate(segment_labels)]))
		# print 's_p', np.sum(np.log([self.transition_matrix[i,j] for i,j in zip(segment_labels[:-1], segment_labels[1:])]))

		psi = np.array([
						np.sum(np.log([unary[i,y_] for i,y_ in enumerate(segment_labels)])),
						np.sum(np.log([self.transition_matrix[i,j] for i,j in zip(segment_labels[:-1], segment_labels[1:])])),
						])
	...
	def inference(self, x, w, relaxed=None,
					min_seg_length=1, max_seg_length=25,
					n_interval=1, loss=0):
		'''
		min_seg_length :
		max_seg_length :
		...
		semi_markov_pairwise = w_pairwise*np.log(self.transition_matrix)
		...
		while idx_start < data_length - 1:
			...
			potential_semi_markov_unaries = w_unary*np.log(self.clf.predict_proba(semi_markov_histograms))
...
# class BinarySVM(CRF):
	...
	# 			semi_markov_unary_hists = self.chi2_sampler.transform(semi_markov_histograms)
	# 			semi_markov_unaries += [self.clf.predict_proba(semi_markov_unary_hists)]
	...
	# 					+ w_unary*semi_markov_unaries
	# 					+ w_pairwise*np.log(self.transition_matrix[state])
	# 					)
	...
	# 			# Get the best transition for each state
	# 			current_class[idx_start,state] = np.argmax(score_tmp,0)
	# 			next_boundary[idx_start,state] = idx_end_candidates[idx_tmp[current_class[idx_start,state]]]
	...
	# 		except:
	# 			embed()
	# 		segments[i,1] = current_class[segments[i-1,0],segments[i-1,1]]
	# 		i += 1
